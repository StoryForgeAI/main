...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      setMsg({ text: "User already registered!", type: "error" });
    } else {
      setMsg({ text: "Success! Please check your email for confirmation.", type: "success" });
    }
    setLoading(false);
  };

  // Google Auth indítása
    ...
    <main style={styles.container}>
      <div style={styles.card}>
        ...
        <form onSubmit={handleRegister} style={styles.form}>
          ...
          {msg.text && (
            <p style={{ color: msg.type === "success" ? "#10b981" : "#ef4444", fontWeight: 'bold', fontSize: '14px' }}>
              {msg.text}
            </p>
          )}

          <button disabled={loading} style={styles.btn}>{loading ? "..." : "SIGN UP"}</button>
        </form>
        ...
        <button onClick={handleGoogleAuth} style={styles.googleBtn}>
          <img src="https://www.google.com/favicon.ico" width="18" alt="G" /> Sign up with Google
        </button>
        
        <p style={styles.footerText}>
          Do you have an account? <a href="/login" style={styles.link}>Login!</a>
        </p>
      </div>
    </main>
  );
